# Linux bash script to help discover bad wordpress plugin by bulk/mass renaming plugin folders and then renaming back one by one,
# so user can check site after each rename if site started/stopped working after, discovering bad plugin.
# USAGE:
# good to have backup of the site/server before running, though script should be safe.
# edit this script by updating variable "dir" a few lines below
...
dir=/home/femhhskz/public_html/domain.com/subdomain/wp-content/plugins
string=6798
#do not edit string, just random
